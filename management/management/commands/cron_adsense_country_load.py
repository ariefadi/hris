...
def fetch_adsense_traffic_per_country_domain(user_mail, start_date, end_date, site_filter='%'):
    return fetch_adsense_traffic_per_country_domain_advanced(
        user_mail,
        start_date,
        end_date,
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if total_insert:
            ...
            try:
                self.stdout.write(self.style.WARNING(
                    f"Sync ClickHouse: log_adsense_country since={start_date} (delete lalu insert)"
                ))
                call_command(
                    'sync_clickhouse',
                    tables='log_adsense_country',
                    since=start_date,
                )
            except Exception as e:
                self.stdout.write(self.style.ERROR(f"Gagal sync ClickHouse log_adsense_country: {e}"))
