                ...
                try:
                    ...
                    for item in res.get('data', []) or []:
                        try:
                            ...
                            ad_requests_coverage = _pct_to_float(item.get('ad_requests_coverage', 0.0) or 0.0)
                            ...
                            record = {
                                ...
                                'data_adsense_cpc': float(cpc),
                                'data_adsense_cpm': float(cpm),
                                'data_adsense_page_views': page_views,
                                'data_adsense_page_views_rpm': float(page_views_rpm_idr),
                                'data_adsense_ad_requests': ad_requests,
                                'data_adsense_ad_requests_coverage': float(ad_requests_coverage),
                                'data_adsense_active_view_viewability': float(_pct_to_float(item.get('active_view_viewability', 0.0))),
                                'data_adsense_active_view_measurability': float(_pct_to_float(item.get('active_view_measurability', 0.0))),
                                'data_adsense_active_view_time': float(_to_float(item.get('active_view_time', 0.0))),
                                'data_adsense_revenue': float(revenue_idr),
                                'mdb': '0',
                                'mdb_name': 'Cron Job',
