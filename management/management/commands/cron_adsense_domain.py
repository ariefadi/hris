...
def fetch_adsense_traffic_per_domain(user_mail, start_date, end_date, site_filter='%'):
    try:
        ...
        for row in report.get('rows', []) or []:
            ...
            ad_unit_name = cells[0].get('value') if len(cells) > 0 else 'Unknown'
            impressions = int(float(cells[1].get('value'))) if len(cells) > 1 else 0
            clicks = int(float(cells[2].get('value'))) if len(cells) > 2 else 0
            earnings = float(cells[3].get('value')) if len(cells) > 3 else 0.0
            ...
            results.append(
                {
                    ...
                    'revenue': float(v.get('earnings', 0.0) or 0.0),
                }
            )
