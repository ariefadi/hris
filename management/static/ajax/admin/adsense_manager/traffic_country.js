...
$(document).ready(function () {
    ...
    function adsenseTrafficCountryPdfWidths(headers) {
        if (!headers || headers.length !== 8) return null;
        return [148, 40, 98, 88, 68, 82, 82, 100];
    }
    ...
    function getExportMetaTrafficCountry() {
        ...
        return {
            ...
            filenameBase: 'adsense_traffic_country_' + String(start || 'data').replace(/-/g, '') + '_' + String(end || 'data').replace(/-/g, '')
        };
    }
    ...
    function initializeDataTable(data) {
        ...
        window.adsenseTrafficCountryDt = table;
    ...
    function createCountryMap(data) {
        ...
        try {
            ...
            loadWorldMapTopology().then(function (topology) {
                ...
                $('#worldMapAdsense').html('<div class="adsense-traffic-empty" style="padding:60px 24px;"><div class="adsense-traffic-empty-title">Gagal memuat peta</div><div>' + escapeHtml(mapErr.message || String(mapErr)) + '</div></div>');
            });
        } catch (error) {
