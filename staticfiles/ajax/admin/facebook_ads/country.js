...
$().ready(function () {
    ...
    // Inisialisasi select2 untuk filter negara
...
function table_data_per_country_facebook(tanggal_dari, tanggal_sampai, data_sub_domain, data_account) {
    ...
    // Ambil nilai filter negara yang dipilih
    var selected_countries = $('#select_country').val();
    var countries_param = '';
    if (selected_countries && selected_countries.length > 0) {
        countries_param = '&countries=' + selected_countries.join(',');
    }
    
    $.ajax({
        url: '/management/admin/page_per_country_facebook?tanggal_dari='+tanggal_dari+'&tanggal_sampai='+tanggal_sampai+'&data_sub_domain='+data_sub_domain+'&data_account='+data_account+countries_param,
        method: 'GET',
        dataType: 'json',
        beforeSend: function () {
            $('#overlay').fadeIn(500);
...
// Fungsi untuk memuat data negara ke select2
function loadCountriesForSelect2() {
    $.ajax({
        ...
        beforeSend: function() {
            ...
            $('#select_country').select2('destroy');
            $('#select_country').html('<option value="">Loading...</option>');
            $('#select_country').select2({
                placeholder: 'Loading...',
                allowClear: true,
                width: '100%',
        ...
        success: function(response) {
            ...
            $('#select_country').append('<option value=""></option>');
            ...
            if (response.status === 'success' && response.countries) {
                ...
                // Fallback ke daftar negara umum jika gagal
...
// Fungsi fallback untuk negara umum
